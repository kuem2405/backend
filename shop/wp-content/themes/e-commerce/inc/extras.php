...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	 ...
	 */
	function e_commerce_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'e-commerce' ), max( $paged, $page ) );
		}

		return $title;
	}
	add_filter( 'wp_title', 'e_commerce_wp_title', 10, 2 );
	 ...
	 */
	function e_commerce_render_title() {
		?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
		<?php
	}
	add_action( 'wp_head', 'e_commerce_render_title' );
endif;
